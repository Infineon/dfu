...
/* USER CONFIGURABLE: The USB device COM port data endpoint maximum packet size */
        ...
        #define CY_DFU_USB_CDC_ENDPOINT_MAX_PACKET   (USB_HS_BULK_MAX_PACKET_SIZE)
    #endif /* #if defined (CY_IP_MXUSBFS) */
...
#elif defined (CY_IP_MXS22USBHS)
    #define CY_DFU_USB_HID_ENDPOINT_SIZE         (USB_HS_INT_MAX_PACKET_SIZE)
#endif /* #if defined (CY_IP_MXUSBFS) */
...
static uint8_t OutBuffer[CY_DFU_USB_CDC_ENDPOINT_MAX_PACKET];

/* The pointer to the initialization/de-initialization callback function
 * USB hardware
 */
static Cy_DFU_TransportUsbCdcCallback usb_callback;

/*******************************************************************************
* Function Name: Cy_DFU_TransportUsbCdcConfig
****************************************************************************//**
*
...
void USB_CDC_CyBtldrCommStart(void)
{
    ...
    {
        usb_callback(CY_DFU_TRANSPORT_USB_CDC_INIT);
        ...
        (void) memset(&EPBulkIn, 0x0, sizeof(EPBulkIn));
        (void) memset(&EPBulkOut, 0x0, sizeof(EPBulkOut));
        (void) memset(&EPIntIn, 0x0, sizeof(EPIntIn));
        (void) memset(&InitData, 0x0, sizeof(InitData));
        ...
        InitData.EPIn  = (uint8_t)USBD_AddEPEx(&EPBulkIn, NULL, 0);
        ...
        InitData.EPOut = (uint8_t)USBD_AddEPEx(&EPBulkOut, OutBuffer, sizeof(OutBuffer));
        ...
        InitData.EPInt = (uint8_t)USBD_AddEPEx(&EPIntIn, NULL, 0);

        /* Adds a CDC class to the stack */
        hInst = USBD_CDC_Add(&InitData);

        usb_callback(CY_DFU_TRANSPORT_USB_CDC_ENABLE);
    }
}
...
void USB_CDC_CyBtldrCommStop(void)
{
    ...
    if (NULL != usb_callback)
    {
        usb_callback(CY_DFU_TRANSPORT_USB_CDC_DISABLE);
        usb_callback(CY_DFU_TRANSPORT_USB_CDC_DEINIT);
    }
}
...
cy_en_dfu_status_t USB_CDC_CyBtldrCommRead(uint8_t pData[], uint32_t size, uint32_t *count, uint32_t timeout)
{
    ...
    if ((USBD_GetState() & (USB_STAT_CONFIGURED | USB_STAT_SUSPENDED)) == USB_STAT_CONFIGURED)
    {
        ...
        int32_t retVal = USBD_CDC_Receive(hInst, pData, size, (int32_t)timeout);

        /* Data received successfully */
        if (retVal > 0)
        {
            *count = (uint32_t)retVal;
...
cy_en_dfu_status_t USB_CDC_CyBtldrCommWrite(const uint8_t pData[], uint32_t size, uint32_t *count, uint32_t timeout)
{
    ...
    if ((USBD_GetState() & (USB_STAT_CONFIGURED | USB_STAT_SUSPENDED)) == USB_STAT_CONFIGURED)
    {
        ...
        if (USBD_CDC_WaitForTX(hInst, timeout) == 0)
        {
            /* Write data to the Host (blocking with timeout) */
            int32_t retVal = USBD_CDC_Write(hInst, pData, size, (int32_t)timeout);
